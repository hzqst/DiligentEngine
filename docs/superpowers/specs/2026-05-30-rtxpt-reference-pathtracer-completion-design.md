...
- RTXPT: see RTXPT-fork's `style_and_conventions` memory — PascalCase classes/functions, `m_`/`g_`/`k`/`c_` prefixes, HLSL namespaces + inline helpers + traditional include guards (DXC compatibility), NVIDIA copyright, no clang-format.
